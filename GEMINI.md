...
* **Canonical OpenAPI spec (machine source of truth)**:
  * Served live at **[https://slopcafe.com/openapi.json](https://slopcafe.com/openapi.json)** — OpenAPI **3.1.0**, `info.version` tracked in `tool/CONTRACT_VERSION` (currently **0.5.0**). The app's models + `ErrorCode` are generated from a pinned copy at `tool/openapi.json`. See the **API layer** section below.
...
* **Pinned spec**: `tool/openapi.json` — an exact copy of what prod serves at
  ...
  contract definition now also excludes expired keys (no schema diff).
  ```sh
