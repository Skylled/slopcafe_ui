...
* **`2.0.0` FROZE on 2026-07-25**, and that changes the re-pin cadence. Until
  ...
  * **Caveat for the next session:** the living reference (`slopcafe-http-api`,
    doc version 34) still carries its *"`2.0.0` IS CURRENTLY AN OPEN MAJOR —
    pin exactly, not with a caret"* paragraph. That prose lags the freeze. The
    doc itself notes the version bump is "a human obligation, not
    test-enforced", so treat the numeral and the narrative as capable of
    drifting apart, and don't revert this note on the strength of that
    paragraph alone.
...
* **Smoke test**: [tool/smoke_test.dart](file:///Users/kyle/Repos/slopcafe_ui/tool/smoke_test.dart)
  ...
  sentinel could otherwise hide a contract change). The backfill sweep and the
  three slug mutators are deliberately **not** exercised: this script runs
  against prod and every one of them writes.
