...
* **Generated but deliberately UNCONSUMED after the 2.0.0 adoption.** These are
  ...
  * **Link graph** — `DocumentLinksResponse` / `OutboundLink` (`GET /d/{id}/links`),
    `OrphanDocumentsResponse` (`GET /admin/links/orphans`) and
    `LinksBackfillResponse` (`POST /admin/links/backfill`). Classes exist; nothing
    imports them.
  * **Change feed** — the `order=updated` and `updated_since` query params on
    `GET /admin/documents` / `GET /d` (and `updated_since` on search). Query params,
    so the generator emits nothing for them; the app still lists in the default
    `created` order. **Gotcha for whoever picks this up:** a cursor *encodes the
    ordering that minted it*. Replaying a `created`-ordered cursor under
    `order=updated` (or the reverse) is a hard `400 bad_cursor`, **not** a silent
    re-sort — so the ordering has to be part of the pagination state, and switching
    it must drop the cursor and restart the walk. `updated_since` is **inclusive**
    on purpose (a resuming consumer re-delivers the boundary row rather than
    skipping one that shares a millisecond), so a consumer must be idempotent per
    row. The `updated_at` / `current_version_at` fields the feed sorts on are
    already on `DocumentListing`.
...
* **Smoke test**: [tool/smoke_test.dart](file:///Users/kyle/Repos/slopcafe_ui/tool/smoke_test.dart)
  ...
  the same two numbers, asserting `servedVer` / `hasUnpublishedWork` both ways). Run `dart run tool/smoke_test.dart`; set
