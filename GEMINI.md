...
- **Local Persistence**: 
  - `flutter_secure_storage` (v10; secure storage of the **saved set of
    ...
    auto-migrate on first access, so no `AndroidOptions` are set.
  - Custom SQLite or local file-based database for offline document caching,
    **namespaced per instance** (`public_id` is a per-deployment fact).
...
* **[lib/core/document_cache.dart](file:///Users/kyle/Repos/slopcafe_ui/lib/core/document_cache.dart)**
  ...
  * Encapsulates saving and loading the documents metadata listing (`documents_list.json`), checking if specific document versions exist in the cache, and scanning the cache directory for version resolution.
  * **Namespaced per instance**: every file lives under `slopcafe_doc_cache/<instance-id>/`. A `public_id` is a fact about *one* deployment, so a flat shared cache would serve one deployment's bytes under another's name whenever two instances minted the same id. `deleteNamespace(id)` drops an instance's cache when it is removed or re-pointed; loose files left by a pre-namespace build are pruned from the cache root once per process.
